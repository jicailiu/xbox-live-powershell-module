...
namespace SessionHistoryViewer
{
    public partial class Form1 : Form
    {
        ...
        private static bool VLeftScrollInProgress = false;
        private static bool VRightScrollInProgress = false;
        ...
        private async Task<Tuple<HttpStatusCode, string>> QueryForHistoricalSessionDocuments(XSTSTokenResponse xToken, QueryBundle queryBundle, string continuationToken)
        {
        ...
        private async void listView1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (!ShowingOfflineSession)
            {
                ...
                XSTSTokenResponse xToken = await tm.GetToken("http://xboxlive.com");

                if (xToken == null)
                {
                    MessageBox.Show("Could not obtain user xtoken");
                    return;
                }
                ...
                if (listView1.SelectedIndices.Count == 1)
                {
                    ...
                    queryResponse = await QueryForDocSessionHistoryAsync(xToken, listView1.Items[index].SubItems[0].Text, listView1.Items[index].SubItems[1].Text).ConfigureAwait(true);
                }
        ...
        private async Task<Tuple<SessionHistoryDocumentResponse, string>> QueryForDocSessionHistoryAsync(XSTSTokenResponse xToken, string sessionName, string branch)
        {
        ...
        private async Task<Tuple<string, string>> QueryForDocSessionHistoryChangeAsync(string sessionName, string branch, long changeNumber)
        {
            ...
            if (!SnapshotCache.TryGetSnapshot(hashKey, out snapshot))
            {
                ...
                if (xToken == null)
                {
                    return new Tuple<string, string>(null, "Could not obtain user xtoken");
                }
        ...
        {
            if (queryBundle.Sandbox != tm.sandbox)
            {
                tm.signout();
                tm.sandbox = tbSandbox.Text;
            }
            ...
            if (xToken == null)
            {
                tm.sandbox = string.Empty;
                MessageBox.Show("Could not obtain user xtoken.\n\nPossible reasons include:\n\n1. You specified an incorrect userId or passworrd. (try again)\n2. You don't have runtime access to the requested sandbox\n3. The XSTS service or related service is not working properly.", "Access Denied");
                return;
            }
            ...
            do
            {
                response = await QueryForHistoricalSessionDocuments(xToken, queryBundle, continuationToken);
            ...
            if (response.Item1 != HttpStatusCode.OK)
            {
                ...
                {
                    tm.sandbox = null; // nulling the sandbox will cause a new token to be obtained on a subsequent query
        ...
        private void tbSandbox_TextChanged(object sender, EventArgs e)
        {
            userSettings.Sandbox = tbSandbox.Text;
        }

        private void tbScid_TextChanged(object sender, EventArgs e)
        {
            userSettings.Scid = tbScid.Text;
        }

        private void tbTemplateName_TextChanged(object sender, EventArgs e)
        {
            userSettings.TemplateName = tbTemplateName.Text;
        }

        private void tbQueryKey_TextChanged(object sender, EventArgs e)
        {
        ...
        private async void saveSessionHistoryToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            Tuple<SessionHistoryDocumentResponse, string> queryResponse = await QueryForDocSessionHistoryAsync(xToken, document.SessionName, document.Branch);
